...
contract RwaLending is IERC1155Receiver, OwnerIsCreator, ReentrancyGuard {
    ...
    struct LoanDetails {
        uint256 erc1155AmountSupplied;//抵押进来tokenid多少数量
        uint256 usdcAmountLoaned;//贷出多少
        uint256 usdcLiquidationThreshold;//usdc清算阈值
    }
    ...
    constructor(
        ...
        i_ltvLiquidationThreshold = 75;//清算门槛
    }
    ...
    function borrow(
        ...
        uint256 loanAmount = (normalizedValuation * i_ltvInitialThreshold) / 100; //能贷出抵押标的价值60%
        if (loanAmount < minLoanAmount) revert SlippageToleranceExceeded();

        uint256 liquidationThreshold = (normalizedValuation * i_ltvLiquidationThreshold) / 100;//清算价值
    ...
    function liquidate(uint256 tokenId, address borrower) external {
        ...
        if (liquidationThreshold < loanDetails.usdcLiquidationThreshold) {//执行清算
